...
router.get('/', async (req, res) => {
	...
	try {
		...
		if (users === null) {
			...
			return res.render('error', { message: 'Error connecting to MongoDB' });
		}
...
router.post('/setUserPic', upload.single('image'), async (req, res) => {
	try {
		...
		if (validationResult.error) {
			console.log('Joi error:', validationResult.error);
		...
		cloudinary.uploader.upload(
			...
			async (error, result) => {
				if (error) {
					console.log('==== CLOUDINARY USER ERROR ====');
					console.log(error);
				...
				console.log('==== CLOUDINARY USER RESULT ====');
				console.log(result);
				...
				try {
					...
					console.log('Mongo save error:', ex);
		...
		console.log('Outer /setUserPic error:', ex);
...
router.post('/setPetPic', upload.single('image'), async (req, res) => {
	try {
		...
		cloudinary.uploader.upload(
			...
			async (error, result) => {
				if (error) {
					console.log('==== CLOUDINARY PET ERROR ====');
					console.log(error);
					console.log('message:', error.message);
					console.log('http_code:', error.http_code);
					return res.render('error', { message: 'Error uploading image to Cloudinary' });
				...
				console.log('==== CLOUDINARY PET RESULT ====');
				console.log(result);
				...
				try {
					...
					console.log('Mongo pet update result:', updateResult);
					res.redirect(`/showPets?id=${user_id}`);
				} catch (ex) {
					console.log('Mongo pet save error:', ex);
					res.render('error', { message: 'Error saving image to MongoDB' });
				}
			},
			{ public_id: image_uuid }
		...
		console.log('Outer /setPetPic error:', ex);
		res.render('error', { message: 'Error uploading image' });
	}
});
